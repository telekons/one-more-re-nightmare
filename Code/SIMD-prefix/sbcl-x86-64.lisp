...
(defvar *swizzled-names*)
(defun swizzle-8-bits (value)
  (or (gethash value *swizzled-names*)
      (setf (gethash value *swizzled-names*)
            (alexandria:format-symbol nil "SWIZZLED-~a" value))))

(defun translate-scalar-code (code)
  (let* ((*swizzled-names* (make-hash-table))
         (translated (%translate-scalar-code code)))
    `(let ,(loop for original being the hash-keys of *swizzled-names*
                 for swizzled being the hash-values of *swizzled-names*
                 collect `(,swizzled (one-more-re-nightmare.vector-primops:v8-
                                      ,original
                                      ,(find-broadcast #x80))))
       ,translated)))
